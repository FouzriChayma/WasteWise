...
class ComplaintController extends AbstractController
{
    ...
    public function index(ComplaintRepository $complaintRepository, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        // Retrieve the user ID directly from the property (assuming "userId" is the correct property name)
        $userId = $user->getUserIdentifier();

        // Fetch complaints associated with the logged-in user
        $data = $complaintRepository->findBy(['user' => $userId]);
...
public function showUserComplaint(Complaint $complaint, Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    #[Route('/usercomplaint', name: 'complaints_by_user', methods: ['GET'])]
